...
// копировать данные для преобразования информации, std::string_view передаст данные в метод без копирования даже если мы будем использовать стркоове литералы или char массивы.
...
int main() {
	...
	size_t size = str_1.length();
	size_t sizeA = str_1.size();
	std::cout << size << " " << sizeA << std::endl;
